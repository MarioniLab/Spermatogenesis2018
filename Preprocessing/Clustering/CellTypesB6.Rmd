...
markers.broad <- marker.detection(sce.B6, ct)

write.xlsx(markers.broad, file = "../../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/10X/Results/Marker_genes/BroadGroups_B6.xlsx")

colData(sce.B6)$BroadCluster <- ct
```
...
all.sce <- split.sce(sce.B6, groups = unique(ct), colData.name = "BroadCluster")
...
ggplot(data.frame(tSNE1 = reducedDims(sce.B6)$TSNE[,1],
                  tSNE2 = reducedDims(sce.B6)$TSNE[,2],
                  sample = as.factor(colData(sce.B6)$BroadCluster))) +
  geom_point(aes(tSNE1, tSNE2, colour = sample))
...
# Rename SubCluster based on their position on pseudotime
counter <- 3
...
col_vector <- c(brewer.pal(8, name = "Set1"), brewer.pal(8, name = "Set2"),
                                brewer.pal(8, name = "Set3"))
names(col_vector) <- unique(new.clusters)
ggplot(data.frame(tSNE1 = reducedDims(sce.B6)$TSNE[,1],
                  tSNE2 = reducedDims(sce.B6)$TSNE[,2],
...
colData(sce.B6)$SubCluster <- new.clusters
colData(sce.B6)$BroadCluster[new.clusters == "Outliers"] <- "Outliers"
```
...
markers.sub <- marker.detection(sce.B6[,colData(sce.B6)$SubCluster != "Outliers"], 
        colData(sce.B6)$SubCluster[colData(sce.B6)$SubCluster != "Outliers"])

# Write out marker genes
openxlsx::write.xlsx(markers.sub, file = "../../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/10X/Results/Marker_genes/SubGroups.xlsx")

# Save cluster IDs
colData(sce)$BroadCluster <- NA
colData(sce)$BroadCluster[grepl("B6", colData(sce)$Sample)] <- colData(sce.B6)$BroadCluster
colData(sce)$SubCluster <- NA
colData(sce)$SubCluster[grepl("B6", colData(sce)$Sample)] <- colData(sce.B6)$SubCluster
```
...
# Random forrest classifier for cell type detection in other samples

## First for broad groups

```{r}
# Select marker genes
genes <- lapply(markers.broad, function(n){rownames(n)[1:100]})
genes <- as.character(unlist(genes))
genes <- genes[!is.na(genes)]

# Create Test and Trainings dataset
labels <- factor(colData(sce.B6)$BroadCluster, levels = c(1:5, "Outliers"))

set.seed(1234)
sam <- sample(1:ncol(sce.B6), 2000)
train.labels <- labels[sam] 
train.data <- logcounts(sce.B6)[genes,sam]
test.labels <- labels[-sam] 
test.data <- logcounts(sce.B6)[genes,-sam]

# Train the model
Tree.broad <- randomForest(x = t(as.matrix(train.data)), y = train.labels, 
                     xtest = t(as.matrix(test.data)), ytest = test.labels, keep.forest = TRUE)

# Confudion matrix
Tree.broad$confusion

# Save genes for generation in object
Tree.broad$Genes <- genes

saveRDS(Tree.broad, "../../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/data/10X_data/Tree_broad_B6.rds")
```
...
```{r}
# Select marker genes
genes <- lapply(markers.sub, function(n){rownames(n)[1:100]})
genes <- as.character(unlist(genes))
genes <- genes[!is.na(genes)]

# Create Test and Trainings dataset
labels <- factor(colData(sce.B6)$SubCluster, levels = c(1:23, "Outliers"))
...
# Train the model
Tree.sub <- randomForest(x = t(as.matrix(train.data)), y = train.labels, 
                     xtest = t(as.matrix(test.data)), ytest = test.labels, keep.forest = TRUE)
...
# Save genes for generation in object
Tree.sub$Genes <- genes

saveRDS(Tree.sub, "../../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/data/10X_data/Tree_sub_B6.rds")
```
