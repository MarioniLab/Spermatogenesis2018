...
```{r}
cur_genes <- c(rownames(results.DE$Group_Leydig_1_P10)[1:5], 
               rownames(results.DE$Group_Leydig_1_P5)[1:5],
...
rownames(for.heatmap) <- rowData(sce.somatic)$Symbol[match(cur_genes, rowData(sce.somatic)$ID)]
colnames(for.heatmap) <- paste(colData(sce.somatic)$Barcode, 
                               colData(sce.somatic)$Library,
                               colData(sce.somatic)$Sample,
                               colData(sce.somatic)$AnnotatedClusters, sep = "__")
