...
cur_col <- apply(for_heatmap[1:8,], 2, function(n){sum(n > 0)})
...
pheatmap(for_heatmap, cluster_rows = FALSE, cluster_cols = FALSE,
         ...
         show_colnames = FALSE,
         color = viridis(100), annotation_colors = list(cluster = col_vector,
                                    stem_genes = colorRampPalette(c("grey", "red", "black"))(100)))
dev.off()

cur_col <- apply(logcounts(sce.spermatogonia)[rowData(sce.spermatogonia)$Symbol %in% cur_genes[1:8],], 2, function(n){sum(n > 0)})

cur_plot <- ggplot(data.frame(PC1 = pca$x[1:ncol(sce.spermatogonia),1], PC2 = pca$x[1:ncol(sce.spermatogonia),2], col = cur_col)) + 
  geom_point(aes(PC1, PC2, colour = col)) + 
  scale_colour_gradientn(colours = colorRampPalette(c("grey", "red", "black"))(100))

ggsave("../../../../Dropbox (Cambridge University)/SST_spermatocytes/Revisions/Results/New_figures/Supplemental_figures/Fig_S5/Stem_cells.pdf")
