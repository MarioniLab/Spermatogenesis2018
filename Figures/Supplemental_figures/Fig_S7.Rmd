...
```{r}
cur_quant <- read.xlsx("../../../../Dropbox (Cambridge University)/SST_spermatocytes/Revisions/Results/RNAScope/RNAscope_quantification.xlsx",
                       sheet = 4)
```
...
cur_groups <- c("Spermatocytes_1", "Spermatocytes_2", 
                ...
                "Spermatocytes_8", "S1", "S2", "S3", "S4", "S5", "S6", "S7",
                "S8", "S9", "S10", "S11")
...
for(i in cur_groups){
  ...
  p.list[[i]] <- cur_p
  results.DE[[i]] <- cur_markers[cur_markers$FDR < 0.1,]
}
