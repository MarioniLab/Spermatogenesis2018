...
ggplot(data.frame(tsne1 = reducedDims(tsne.somatic)$TSNE[,1],
                  tsne2 = reducedDims(tsne.somatic)$TSNE[,2],
                  gene = logcounts(sce.somatic)[rowData(sce.somatic)$Symbol == "Dlk1",]))               + geom_point(aes(tsne1, tsne2, colour = gene), size = 0.5) +
    scale_color_viridis()
...
results.DE <- list()
cur_groups <- c("Sertoli", "Leydig", 
                "Immature_Leydig_1", "Immature_Leydig_2", 
                "Immature_Leydig_3", "PTM", "Endothelial_cells", 
                "Interstitial_tMg")
for(i in cur_groups){
...
write.xlsx(results.DE, "../../../Dropbox (Cambridge University)/SST_spermatocytes/Revisions/Analysis/Somatic_cells_P5_P10.xlsx")
