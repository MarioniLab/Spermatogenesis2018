...
sample <- ggplot() +
  geom_point(data = data.frame(PC1 = pca$x[1:ncol(sce.spermatogonia),1],
             ...
             aes(PC1, PC2, colour = sample), size = 0.5) + scale_y_reverse() +
...
cluster <- ggplot() +
  geom_point(data = data.frame(PC1 = pca$x[1:ncol(sce.spermatogonia),1],
                  PC2 = pca$x[1:ncol(sce.spermatogonia),2],
                  cluster = clusters),
             aes(PC1, PC2, colour = cluster), size = 0.5) + scale_y_reverse() +
...
cluster_RA <- ggplot() +
   geom_point(data = data.frame(PC1 = pca$x[1:ncol(sce.spermatogonia),1],
                                 PC2 = pca$x[1:ncol(sce.spermatogonia),2],
                                 cluster = clusters),
               aes(PC1, PC2, colour = cluster), size = 1) +
   ...
   geom_point(data = data.frame(PC1 = pca$x[(ncol(sce.spermatogonia)+1):(ncol(sce.spermatogonia)+ncol(sce_early_RA)),1],
                                 PC2 = pca$x[(ncol(sce.spermatogonia)+1):(ncol(sce.spermatogonia)+ncol(sce_early_RA)),2],
                                 cell_type = colData(sce_early_RA)$cell_type),
               aes(PC1, PC2, colour = cell_type), shape = 16, size = 1.8) +
  scale_y_reverse() + scale_color_manual(values = c(col_vector, breaks = c(levels(colData(sce_early_RA)$cell_type),
                                                         levels(cluster))))
...
Gfra1 <- ggplot() +
      geom_point(data = data.frame(PC1 = pca$x[1:ncol(sce.spermatogonia),1],
                                ...
                                Gfra1 = logcounts(sce.spermatogonia)[
                                    rowData(sce.spermatogonia)$Symbol == "Gfra1",]), aes(PC1, PC2, colour = Gfra1), size = 0.5) + scale_y_reverse() + 
                                    scale_colour_viridis()
Id4 <- ggplot() +
      geom_point(data = data.frame(PC1 = pca$x[1:ncol(sce.spermatogonia),1],
                                 PC2 = pca$x[1:ncol(sce.spermatogonia),2],
                                Id4 = logcounts(sce.spermatogonia)[
                                    rowData(sce.spermatogonia)$Symbol == "Id4",]), aes(PC1, PC2, colour = Id4), size = 0.5) + scale_y_reverse() + 
                                    scale_colour_viridis()
...
Nanos3 <- ggplot() +
      geom_point(data = data.frame(PC1 = pca$x[1:ncol(sce.spermatogonia),1],
                                ...
                                Nanos3 = logcounts(sce.spermatogonia)[
                                    rowData(sce.spermatogonia)$Symbol == "Nanos3",]), aes(PC1, PC2, colour = Nanos3), size = 0.5) + scale_y_reverse() + 
                                    scale_colour_viridis()
...
Zbtb16 <- ggplot() +
      geom_point(data = data.frame(PC1 = pca$x[1:ncol(sce.spermatogonia),1],
                                ...
                                Zbtb16 = logcounts(sce.spermatogonia)[
                                    rowData(sce.spermatogonia)$Symbol == "Zbtb16",]), aes(PC1, PC2, colour = Zbtb16), size = 0.5) + scale_y_reverse() + 
                                    scale_colour_viridis()
...
Stra8 <- ggplot() +
      geom_point(data = data.frame(PC1 = pca$x[1:ncol(sce.spermatogonia),1],
                                ...
                                Stra8 = logcounts(sce.spermatogonia)[
                                    rowData(sce.spermatogonia)$Symbol == "Stra8",]), aes(PC1, PC2, colour = Stra8), size = 0.5) + scale_y_reverse() +
                                    scale_colour_viridis()
...
Dmrtb1 <- ggplot() +
      geom_point(data = data.frame(PC1 = pca$x[1:ncol(sce.spermatogonia),1],
                                ...
                                Dmrtb1 = logcounts(sce.spermatogonia)[
                                    rowData(sce.spermatogonia)$Symbol == "Dmrtb1",]), aes(PC1, PC2, colour = Dmrtb1), size = 0.5) + scale_y_reverse() + 
                                    scale_colour_viridis()
...
# Compute pca 
pca.p5 <- prcomp(t(corrected))

# Visualize PCA with marker genes
libraries <- ggplot(data.frame(PC1 = pca.p5$x[,1],
                  PC2 = pca.p5$x[,2],
                  libraries = colData(spermatogonia.P5)$Library)) +
  geom_point(aes(PC1, PC2, colour = libraries)) + 
  scale_color_manual(values = c("#FF9100", "#e31a1c")) 
...
p5.cluster <- ggplot(data.frame(PC1 = pca.p5$x[,1],
            ...
            geom_point(aes(PC1, PC2, colour = clusters), size = 0.5) + 
            scale_colour_manual(values = col_vector)

# Plot marker gene expression

Gfra1.p5 <- ggplot(data.frame(PC1 = pca.p5$x[,1],
                  PC2 = pca.p5$x[,2],
                  Gfra1 = logcounts(spermatogonia.P5)[
                    rowData(spermatogonia.P5)$Symbol == "Gfra1",])) +
  geom_point(aes(PC1, PC2, colour = Gfra1), size = 0.5) + scale_colour_viridis()

Stra8.p5 <- ggplot(data.frame(PC1 = pca.p5$x[,1],
                  PC2 = pca.p5$x[,2],
                  Stra8 = logcounts(spermatogonia.P5)[
                    rowData(spermatogonia.P5)$Symbol == "Stra8",])) +
  geom_point(aes(PC1, PC2, colour = Stra8), size = 0.5) + scale_colour_viridis()

Eif2s2.p5 <- ggplot(data.frame(PC1 = pca.p5$x[,1],
                  PC2 = pca.p5$x[,2],
                  Eif2s2 = logcounts(spermatogonia.P5)[
                    rowData(spermatogonia.P5)$Symbol == "Eif2s2",])) +
  geom_point(aes(PC1, PC2, colour = Eif2s2), size = 0.5) + scale_colour_viridis()
