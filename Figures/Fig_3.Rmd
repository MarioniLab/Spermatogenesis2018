...
sce.spermatogonia <- sce[,grepl("P5|P10|P15", colData(sce)$Sample) &
                                        grepl("Spermatogonia", colData(sce)$AnnotatedClusters)]
...
# Visualize different types of spermatogonia
ggplot(data.frame(PC1 = reducedDims(sce.spermatogonia)$PCA[,1],
                  PC2 = reducedDims(sce.spermatogonia)$PCA[,2],
                  samples = colData(sce.spermatogonia)$AnnotatedClusters)) +
  geom_point(aes(PC1, PC2, colour = samples)) + 
  scale_color_manual(values = metadata(sce.spermatogonia)$color_vector)
