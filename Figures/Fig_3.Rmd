...
pos.cor <- cors[cors$FDR < 0.1 & cors$rho > 0.3,]
neg.cor <- cors[cors$FDR < 0.1 & cors$rho < -0.3,]
no.cor <- cors[!(cors$gene2 %in% pos.cor$gene2) & !(cors$gene2 %in% neg.cor$gene2), ]
...
pdf("/Users/eling01/Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_3_PosCorrelationHeatmap.pdf", onefile = FALSE)
...
set.seed(1234)
sam <- sample(1:nrow(no.cor), 20)
for.heatmap <- logcounts(sce.spermatocytes)[no.cor$gene2[sam],order(prank[,"rank"])]
colnames(for.heatmap) <- colData(sce.spermatocytes)$Barcode[order(prank[,"rank"])]
pdf("/Users/eling01/Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_3_NoCorrelationHeatmap.pdf", onefile = FALSE)
pheatmap(for.heatmap, show_colnames = FALSE,
         cluster_rows = FALSE, cluster_cols = FALSE, color = viridis(100), 
         labels_row = no.cor$genename[sam], 
         annotation_col = data.frame(row.names = colnames(for.heatmap),
                  cell_type = colData(sce.spermatocytes)$AnnotatedClusters[order(prank[,"rank"])]),
         annotation_colors = list(cell_type = metadata(sce.spermatocytes)$color_vector),
         annotation_row = data.frame(row.names = rownames(for.heatmap),
...
colnames(for.heatmap) <- colData(sce.spermatocytes)$Barcode[order(prank[,"rank"])]
pdf("/Users/eling01/Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_3_NegCorrelationHeatmap.pdf", onefile = FALSE)
pheatmap(for.heatmap, show_colnames = FALSE,
         cluster_rows = FALSE, cluster_cols = FALSE, color = viridis(100), 
