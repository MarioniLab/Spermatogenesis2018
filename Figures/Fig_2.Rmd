...
```{r, visualization}
cur_df <- proportions[c("B6", "P10", "P15", "P20", "P25", "P30", "P35"),c(26, 25, 24, 27, 1:23)]
cur_df.melt <- melt(cur_df)
levels(cur_df.melt$sample) <- rev(c("B6", "P10", "P15", "P20", "P25", "P30", "P35"))
levels(cur_df.melt$group) <- levels(colData(sce.all)$AnnotatedClusters)[c(26, 25, 24, 27, 1:23)]

proportions.p <- ggplot(cur_df.melt) + 
  geom_point(aes(group, rev(sample), size = value, fill = group), shape = 22) +
...
# Save plot
ggsave(filename = "../../../Dropbox (Cambridge University)/SST_spermatocytes/Revisions/Results/New_figures/Fig_2_ProportionsPlot.pdf", proportions.p,
...
sce.B6 <- sce.all[,grepl("B6", colData(sce.all)$Sample) & 
             ...
             c(paste("NA", 1:5, sep = ""), "Outliers", "Endothelial_cells"))]
