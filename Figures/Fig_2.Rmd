...
# Train the classifier on adult B6 - remove outliers
...
# Prediction 
new.data <- log(bulk[cur_markers,] + 1)
new.data.Z <- t(apply(new.data, 1, function(n){(n - mean(n))/sd(n)}))

pred.bulk <- predict(object = Tree, newdata = t(new.data.Z), type="prob")
pred.bulk <- pred.bulk[rownames(pred.bulk) != "do18154",]

meta <- meta[rownames(meta) != "DO18154",]

# Collect day info on libraries
df <- data.frame(library = rownames(pred.bulk),
                 day = meta$`Age.(days)`[order(rownames(meta))])

pdf("/Users/nils/Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_2/Fig_2_bulkmapping_heatmap.pdf", 
    onefile = FALSE)
...
# Map P10 sample and bulk P6-P15

```{r bulk mapping}
# Load marker genes
marker.genes.P10 <- readRDS("/Users/nils/Dropbox (Cambridge University)/SST_spermatocytes/Analysis/10X/Results/Marker_genes/Marker_genes_P10.rds")
cur_markers <- as.character(unlist(lapply(marker.genes.P10, function(n){rownames(n)[1:50]})))
cur_markers <- cur_markers[cur_markers %in% rownames(bulk)]
...
labels <- factor(colData(sce.P10)$AnnotatedClusters,
                    levels = levels(colData(sce.P10)$AnnotatedClusters)[
                      levels(colData(sce.P10)$AnnotatedClusters) %in%
                        unique(colData(sce.P10)$AnnotatedClusters)])

# Create training and testing datasets
set.seed(1234)
sam <- sample(1:ncol(sce.P10), 2000)
#train.data <- data.frame(t(as.matrix(logcounts(sce.B6)[cur_markers,sam])))
#train.data$labels <- labels[sam] 
#test.data <- data.frame(t(as.matrix(logcounts(sce.B6)[cur_markers,-sam])))
#test.data$labels <- labels[-sam] 

train.data <- as.matrix(logcounts(sce.P10)[cur_markers,sam])
train.labels <- labels[sam] 
test.data <- as.matrix(logcounts(sce.P10)[cur_markers,-sam])
test.labels <- labels[-sam] 
...
# Prediction 
new.data <- log(bulk[cur_markers,1:10] + 1)
new.data.Z <- t(apply(new.data, 1, function(n){(n - mean(n))/sd(n)}))
...
# Collect day info on libraries
df <- data.frame(library = rownames(pred.bulk),
                 day = meta$`Age.(days)`[1:10])

pdf("/Users/nils/Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_2/Fig_2_bulkmapping_heatmap_P10.pdf", 
    onefile = FALSE)
pheatmap(pred.bulk[order(df$day),], cluster_cols = FALSE, cluster_rows = FALSE,
