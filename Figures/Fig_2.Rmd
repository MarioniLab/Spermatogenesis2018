...
#mat <- matrix(NA, nrow = ncol(bulk), ncol = length(marker.genes))
#rownames(mat) <- colnames(bulk)
#colnames(mat) <- names(marker.genes)
#for(i in names(marker.genes)){
#  cur_genes <- rownames(marker.genes[[i]])[1:50]
#  cur_genes <- cur_genes[cur_genes %in% rownames(bulk)]
#  mat[,i] <- colSums(log2(bulk[cur_genes,] + 1))
#}
