...
cur_meta <- metadata(sce.all)$corrected[,!(colData(sce.all)$Sample %in% c("Tc1", "Tc0"))
...
# Visualize P10 and P15 sample

```{r}
select <- grepl("P10|P15", colData(sce.all)$Sample)
...
# Collect marker genes for somatic cells in P10 and P15

```{r}
cur_sce <- sce.all[,select & !(grepl("Spermatocytes", colData(sce.all)$AnnotatedClusters))]
