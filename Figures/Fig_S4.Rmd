...
sce.P10.15 <- sce[,grepl("P1", colData(sce)$Sample)]
sce.P10.15 <- normalize(sce.P10.15)

sce.spermatogonia.p10.15 <- sce.P10.15[,
          colData(sce.P10.15)$AnnotatedClusters == "Spermatogonia"]
sce.spermatogonia.p10.15 <- normalize(sce.spermatogonia.p10.15)

# Calculate HVG
HVgenes.P10.15 <- HVG(sce.spermatogonia.p10.15)

# PCA
pca.P10.15 <- prcomp(t(logcounts(sce.spermatogonia.p10.15)[HVgenes.P10.15,]))
...
# Save rotations of PC1
rots.PC1 <- data.frame(row.names = rownames(pca.P10.15$rotation),
                       rot.PC1 = pca.P10.15$rotation[,1],
                       genename = rowData(sce.spermatogonia.p10.15)$Symbol[
                         match(rownames(pca.P10.15$rotation),
                               rowData(sce.spermatogonia.p10.15)$ID)])
rots.PC1 <- rots.PC1[order(rots.PC1$rot.PC1, decreasing = TRUE),]
write.xlsx(rots.PC1, "/Users/ernst01/Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_S3/Rotations_PC1_spermatogonia_P10_P15.xlsx")
