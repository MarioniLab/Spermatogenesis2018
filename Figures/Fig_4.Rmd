...
pdf("../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_4/Fig_4_MarkerGenes.pdf", onefile = FALSE, height = 5, width = 10)
...
for(i in 1:nrow(df.stats)){
  ...
  cur_mat[1,1] <- sum(rowData(cur_sce)$Symbol %in% as.character(df.fertility$Genename) & 
                        rowData(cur_sce)$Symbol %in% 
                        marker.genes[[rownames(df.stats)[i]]]$GeneName)
  cur_mat[1,2] <- sum(rowData(cur_sce)$Symbol %in% as.character(df.fertility$Genename) & 
                        !(rowData(cur_sce)$Symbol %in% 
                        marker.genes[[rownames(df.stats)[i]]]$GeneName))
  cur_mat[2,1] <- sum(!(rowData(cur_sce)$Symbol %in% as.character(df.fertility$Genename)) & 
                        rowData(cur_sce)$Symbol %in% 
                        marker.genes[[rownames(df.stats)[i]]]$GeneName)
  ...
  df.stats[i,1] <- test$p.value
}

df.stats[,2] <- p.adjust(df.stats[,1], method = "BH")
...
p.stats <- ggplot(df.stats.red) + geom_col(aes(Celltype, -log10(V2), 
                                           fill = Celltype)) +
