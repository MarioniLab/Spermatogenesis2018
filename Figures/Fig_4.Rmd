...
```{r PT}
sce.spermatocytes <- sce[, colData(sce)$AnnotatedClusters %in% levels(colData(sce)$AnnotatedClusters)[2:8]]
...
p.stats <- ggplot(df.stats.red) + geom_col(aes(Celltype, -log10(BHvalue), 
                                           fill = Celltype)) +
