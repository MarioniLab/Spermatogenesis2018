...
Hist.all <- read.xlsx("../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_4/Mouse_histones.xlsx")
Hist.all <- Hist.all[match(unique(Hist.all$Ensembl.Gene.ID),
                           Hist.all$Ensembl.Gene.ID),]
rownames(Hist.all) <- Hist.all$Ensembl.Gene.ID

# Collect data for heatmap plotting
for.heatmap <- logcounts(sce.spermatids)[rowData(sce.spermatids)$ID %in% Hist.all$Ensembl.Gene.ID,
...
pdf("../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_4/Fig_4_HistoneVariants.pdf", onefile = FALSE)
pheatmap(for.heatmap[cors$gene2,], show_colnames = FALSE,
         ...
         annotation_col = data.frame(row.names = colnames(for.heatmap),
                  cell_type = colData(sce.spermatids)$AnnotatedClusters[order(prank[,"rank"],
                      decreasing = TRUE)]),
         labels_row = Hist.all[cors$gene2,"Gene.name"],
         annotation_colors = list(cell_type = metadata(sce.spermatids)$color_vector), 
         scale = "row",
         annotation_row = data.frame(row.names = cors$gene2,
                                     Testis_specific = Hist.all[cors$gene2,"X7"]))
dev.off()
```
...
ggsave("../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_4/NoGenes.pdf", 
...
pdf("../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_4/Fig_4_TranscriptionalShotdown.pdf", onefile = FALSE)
...
write.xlsx(cors, "../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_4/CorrrelationNoGenes.xlsx")

```

# Visualize infertility genes

```{r}
# Read in the data from IMPC
infertility <- read.xlsx("../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_4/male_infertility.xlsx")
```
