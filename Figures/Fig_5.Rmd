...
df <- data.frame(library = colnames(bulk),
                 day = meta$`Age.(days)`[order(rownames(meta))])

# Generate heatmap for display
for.heatmap <- matrix(0, ncol = ncol(bulk), nrow = length(cur_genes))
rownames(for.heatmap) <- cur_genes
colnames(for.heatmap) <- colnames(bulk)
for.heatmap[!is.na(match(cur_genes, rownames(bulk))),] <- 
  bulk[match(cur_genes, rownames(bulk))[!is.na(match(cur_genes, rownames(bulk)))],]

pdf("/Users/nils/Dropbox (Cambridge University)/SST_spermatocytes/Analysis/Figures/Fig_5/BulkExpression.pdf",
    width = 10, height = 15, onefile=FALSE)
pheatmap(log10(for.heatmap[,order(df$day)] + 1), cluster_cols = FALSE, 
         cluster_rows = FALSE, show_rownames = FALSE,
         color = viridis(100),
         border_color = NA, 
