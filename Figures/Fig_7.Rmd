...
write.xlsx(out.df, "../../../Dropbox (Personal)/Tc1_meiotic_silencing/Revisions/Figures/Supplemental Tables/Table_S8_bulkDetection.xlsx")
```
...
# Klf5
Klf5.tsne <- ggplot(data.frame(tSNE1 = reducedDims(sce)$TSNE[,1],
                      tSNE2 = reducedDims(sce)$TSNE[,2],
                      Gene = logcounts(sce)[rowData(sce)$Symbol == "Klf5",])) +
      geom_point(aes(tSNE1, tSNE2, colour = Gene)) + 
      scale_color_viridis() 

Klf5.box <-  ggplot(data.frame(value = logcounts(sce)[rowData(sce)$Symbol == "Klf5",],
                      cluster = colData(sce)$AnnotatedClusters)) +
...
# Save figure
final <- plot_grid(p.clusters, Klf5.tsne, Klf5.box,
                   ncol = 3, nrow = 1)
ggsave("../../../Dropbox (Cambridge University)/SST_spermatocytes/Revisions/Results/New_figures/Fig_7/Klf5.pdf", final, width = 15, height = 5)
```
