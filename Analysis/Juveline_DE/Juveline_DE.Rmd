...
genes <- intersect(rownames(sce.adult), rownames(sce.P10))
cur_sce <- cbind(sce.adult[genes,], sce.P10[genes,])
...
# P10
#P10.genes <- c("Rpl41", "Rpl37", "Rpl23", "Rps27", "Rps29")

# Adult
#adult.genes <- c("Prm1", "Prm2", "Tnp1", "Tnp2", "Prm3")


cur_markers <- findMarkers(cur_sce[,colData(cur_sce)$Cluster == "1" | colData(cur_sce)$Cluster == "P10_1"],
                           clusters = ifelse(grepl("B6", colData(cur_sce)$Sample[
                             colData(cur_sce)$Cluster == "1" | colData(cur_sce)$Cluster == "P10_1"]), "Adults",
                             "P10"),
                           design = as.matrix(t(logcounts(cur_sce)[rowData(cur_sce)$ID[match(c(P10.genes, adult.genes), rowData(cur_sce)$Symbol)],colData(cur_sce)$Cluster == i])))
...
markers.adult$Genename <- rowData(sce.P10)$Symbol[match(rownames(markers.adult),
                                                                 rowData(sce.P10)$ID)]
results.DE[[paste("Group_1_P10", sep = "")]] <- markers.P10
results.DE[[paste("Group_1_Adult", sep = "")]] <- markers.adult
...
results.GO[[paste("Group_1_P10", sep = "")]] <- enriched.GO
...
# Write out table
write.xlsx(results.DE, "/Users/eling01/Dropbox (Cambridge University)/SST_spermatocytes/Analysis/10X/Results/Marker_genes/Juvenile_Adult_comparison/P10_adult_singleGenes.xlsx")

write.xlsx(results.GO, "/Users/eling01/Dropbox (Cambridge University)/SST_spermatocytes/Analysis/10X/Results/Marker_genes/Juvenile_Adult_comparison/P10_adult_GO.xlsx")
```
