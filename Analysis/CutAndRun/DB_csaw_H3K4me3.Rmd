...
mat.K4 <- matrix(data = NA, ncol = length(seqlevels(bins)), nrow = 2)
rownames(mat.K4) <- c("Spermatocytes", "Spermatids")
colnames(mat.K4) <- seqlevels(bins)
...
for(i in seqlevels(bins)){
  ...
  mat.K4[1,i] <- mean(adjc[as.vector(cur_windows),1])
  mat.K4[2,i] <- mean(adjc[as.vector(cur_windows),2])
}
...
ggplot(cur_df) + geom_point(aes(Var2, value, colour = Var1), size = 2) + 
  ggtitle("H3K4me3") + scale_color_aaas()
...
saveRDS(file="../../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/CnR/DB/H3K4me3/H3K4me3_1000_normReads.rds", list(windows=rowRanges(out), 
    normReads=adjc)) 
```
...
```{r}
cur_bins <- windowCounts(bam.files, param=param, width=1000, bin=TRUE)
cur_bins <- normOffsets(cur_bins)
tmp <- asDGEList(cur_bins)
adjc <- cpm(tmp, log=TRUE, prior.count=0.5)
colnames(adjc) <- paste(treatment, batch, sep = "_")
saveRDS(file="../../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/CnR/DB/H3K4me3/H3K4me3_1000kb_bins_normReads.rds", list(windows=rowRanges(cur_bins), 
    normReads=adjc)) 
```
...
```{r}
saveRDS(file="../../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/CnR/DB/H3K4me3/H3K4me3_1000.rds", list(windows=rowRanges(filtered), 
