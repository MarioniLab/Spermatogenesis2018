...
cur_res.prom <- biomaRt::getSequence(chromosome = as.character(seqnames(cur_prom))[1], 
                                     start = start(ranges(cur_prom))[1],
                                     end = end(ranges(cur_prom))[1], mart=ensembl,
                                     seqType = "cdna", type = "ensembl_gene_id")
cur_res.prom <- cur_res.prom[which.max(sapply(cur_res.prom$cdna, nchar)),]

for(i in 2:length(cur_prom)){
  cur <- biomaRt::getSequence(chromosome = as.character(seqnames(cur_prom))[i], 
                                     start = start(ranges(cur_prom))[i],
                                     end = end(ranges(cur_prom))[i], mart=ensembl,
                                     seqType = "cdna", type = "ensembl_gene_id")
  cur <- cur[which.max(sapply(cur$cdna, nchar)),]
  cur_res.prom <- rbind(cur_res.prom, cur)
}

# Write these sequences in fasta format
write.fasta(sequences = as.list(cur_res.prom$cdna), names = cur_res.prom$ensembl_gene_id, 
              file.out = "../../../../Dropbox (Cambridge University)/SST_spermatocytes/Revisions/Results/Chip-Seq/High_K9_autosomal_prom.fa")
...
cur_res.body <- biomaRt::getSequence(chromosome = as.character(seqnames(cur_gene_body))[1], 
                                     start = start(ranges(cur_gene_body))[1], 
                                     end = end(ranges(cur_gene_body))[1], mart=ensembl,
                                     seqType = "cdna", type = "ensembl_gene_id")
cur_res.body <- cur_res.body[which.max(sapply(cur_res.body$cdna, nchar)),]

for(i in 2:length(cur_gene_body)){
  cur <- biomaRt::getSequence(chromosome = as.character(seqnames(cur_gene_body))[i], 
                                     start = start(ranges(cur_gene_body))[i],
                                     end = end(ranges(cur_gene_body))[i], mart=ensembl,
                                     seqType = "cdna", type = "ensembl_gene_id")
  cur <- cur[which.max(sapply(cur$cdna, nchar)),]
  cur_res.body <- rbind(cur_res.body, cur)
}

# Write these sequences in fasta format
write.fasta(sequences = as.list(cur_res.body$cdna), names = cur_res.body$ensembl_gene_id, 
              file.out = "../../../../Dropbox (Cambridge University)/SST_spermatocytes/Revisions/Results/Chip-Seq/High_K9_autosomal_genebody.fa")
```