...
bam.files <- list.files("../../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/CnR/bam/H3K27ac/", full.names = TRUE,
                        pattern = ".+JP26.+bam$")
```
...
mat.K27 <- matrix(data = NA, ncol = length(seqlevels(bins)), nrow = 2)
rownames(mat.K27) <- c("Spermatocytes", "Spermatids")
colnames(mat.K27) <- seqlevels(bins)
...
for(i in seqlevels(bins)){
  ...
  mat.K27[1,i] <- mean(adjc[as.vector(cur_windows),1])
  mat.K27[2,i] <- mean(adjc[as.vector(cur_windows),2])
}
mat.K27 <- mat.K27[,!grepl("GL|JH", colnames(mat.K27))]
mat.K27 <- mat.K27[,!(colnames(mat.K27) == "MT")]
cur_df <- melt(mat.K27)

ggplot(cur_df) + geom_point(aes(Var2, value, colour = Var1), size = 2) + 
  ggtitle("H3K27ac") + scale_color_aaas()
...
Save normalized reads in all the windows
...
saveRDS(file="../../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/CnR/DB/H3K9me3/H3K9me3_1000_normReads.rds", list(windows=rowRanges(out), 
    normReads=adjc)) 
```
...
```{r}
write.table(file="/Users/nils/Dropbox (Cambridge University)/SST_spermatocytes/Analysis/CnR/DB/H3K27ac/norm_1000.tsv", quote=FALSE, row.names=FALSE, sep="\t",
...
```{r}
saveRDS(file="../../../../Dropbox (Cambridge University)/SST_spermatocytes/Analysis/CnR/DB/H3K27ac/H3K27ac_1000.rds", list(windows=rowRanges(filtered), 
